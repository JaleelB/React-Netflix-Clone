...
router.post("/register", async (request, response, next)=>{
    if(request.body.email && request.body.password){
        ...
        return response.status(500).json("You are missing a username/passowrd. Fill in all details to create an account!");
    }
...
router.post("/login", async (request, response, next)=>{
    if(request.body.email && request.body.password){
        try{
            ...
            if( originalPassword !== request.body.password ) return next(createError(400, "Incorrect Password!")); 
        ...
        return response.status(500).json("You are missing a username/passowrd. Fill in all details to login!");
    }
